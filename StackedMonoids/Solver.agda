...
data Expr (v : ℕ) : Set where
  var : (x : Fin v) → Expr v
  ε   : (x : Fin m) → Expr v
  _[_]_ : (e₁ : Expr v) (∙ : Fin m) (e₂ : Expr v) → Expr v
...
Normal : ℕ → Set
Normal v = NLT (Fin v) (Fin m)

mutual
  ⟦_⟧″ : ∀ {v} → List (Normal v) → (Env v) → Fin m → X
  ⟦ []     ⟧″ Γ n = id n 
  ⟦ x ∷ xs ⟧″ Γ n = ⟦ x ⟧′ Γ ⟨ op n ⟩ ⟦ xs ⟧″ Γ n

  ⟦_⟧′ : ∀ {v} → Normal v → Env v → X
  ⟦ leaf x     ⟧′ Γ = lookup x Γ
  ⟦ branch n t ⟧′ Γ = ⟦ t ⟧″ Γ n

morphism : ∀ {v} (t t′ : List (Normal v)) (Γ : Env v) (n : Fin m)
         → ⟦ t ++ t′ ⟧″ Γ n ≈ (⟦ t ⟧″ Γ n ⟨ op n ⟩ ⟦ t′ ⟧″ Γ n)
...
normalise : ∀ {v} → Expr v → Normal v
normalise     (var x)       = leaf x
normalise     (ε x)         = branch x []
normalise {v} (e₁ [ ∙ ] e₂) = (normalise e₁) ++ (normalise e₂) ⟨ ∙ ⟩
  where open Operations (Fin v) (Fin m) _≟_

homomorphic : ∀ {v} (∙ : Fin m) (nf₁ nf₂ : Normal v) (Γ : Env v) 
            → let open Operations (Fin v) (Fin m) _≟_
              in ⟦ nf₁ ++ nf₂ ⟨ ∙ ⟩ ⟧′ Γ ≈ (⟦ nf₁ ⟧′ Γ ⟨ op ∙ ⟩ ⟦ nf₂ ⟧′ Γ)
...
correct : ∀ {v} (e : Expr v) (Γ : Env v) → ⟦ normalise e ⟧′ Γ ≈ ⟦ e ⟧ Γ
...
open Reflection {z} {c} {ℓ} {Expr} universe var ⟦_⟧ (λ e Γ → ⟦ normalise e ⟧′ Γ) correct 
